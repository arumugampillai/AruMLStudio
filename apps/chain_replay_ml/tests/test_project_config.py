...
class ProjectConfigTests(unittest.TestCase):
    ...
    def test_resolve_chart_dir_repo_root_apps(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            apps = os.path.join(tmp, "apps")
            os.makedirs(os.path.join(apps, "static"))
            self.assertEqual(resolve_chart_dir_from_selection(tmp), os.path.abspath(apps))

    def test_resolve_chart_dir_repo_root_legacy(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
