"""Feature Importance Studio — Milestone 4 UI (sortable comparison table)."""
...
_TABLE_COLS = (
    ...
    ("weight", "Weight", 80),
    ("cover", "Cover", 90),
    ("permutation_mean", "Permutation", 100),
    ("shap_mean_abs", "SHAP", 90),
    ("rank_gain", "Rank Gain", 80),
    ("rank_shap", "Rank SHAP", 80),
    ("rank_delta_gain_shap", "Rank Δ", 70),
)


class FeatureImportanceStudioPanel(ttk.Frame, LazyLoadMixin):
    """Simple Feature Importance Studio: summary + sortable comparison table."""
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(hdr, text="Feature Importance Studio", font=SECTION_FONT).pack(
            side="left"
        )
        ttk.Label(
            hdr,
            text="Analyze an existing model · no retraining",
        ...
        self._tree.tag_configure("disagreement", foreground=COL_WARN)
    ...
    def apply_artifacts(
        ...
        if not loaded:
            self.mark_unavailable("Unavailable — no Importance artifacts.")
            return
        self._apply_result(
    ...
    def _on_load(self, *, quiet: bool = True) -> None:
        ...
        if not name:
            if not quiet:
                messagebox.showwarning(
                    "Feature Importance", "Select a model first.", parent=self
                )
            return
        ...
        if not loaded:
            ...
            if not quiet:
                messagebox.showinfo(
                    "Feature Importance",
                    "No Feature Importance Studio artifacts found for this model.\n"
    ...
    def _apply_filter_sort(self) -> None:
        ...
        if self._top_n_only.get():
            # Top N by gain rank (analytical default), then re-sort for display
            by_gain = sorted(
                self._rows,
                key=lambda r: (
                    int(r["rank_gain"])
            ...
            keep = {str(r.get("feature")) for r in by_gain}
    ...
    def _render_table(self) -> None:
        ...
        for row in self._display_rows:
            ...
            try:
                rank_gain = int(row["rank_gain"]) if row.get("rank_gain") is not None else None
            except (TypeError, ValueError):
                rank_gain = None
            try:
                delta = (
                    int(row["rank_delta_gain_shap"])
                    if row.get("rank_delta_gain_shap") is not None
                    else None
                )
            except (TypeError, ValueError):
                delta = None
            ...
            values = (
                feat,
                fmt_num(row.get("gain"), 4),
                fmt_num(row.get("weight"), 1),
                fmt_num(row.get("cover"), 2),
                fmt_num(row.get("permutation_mean"), 6),
                fmt_num(row.get("shap_mean_abs"), 6),
                fmt_val(row.get("rank_gain")),
                fmt_val(row.get("rank_shap")),
                fmt_val(row.get("rank_delta_gain_shap")),
            )
    ...
    def _on_export(self) -> None:
        ...
        path = filedialog.asksaveasfilename(
            ...
            filetypes=[("CSV", "*.csv")],
            initialfile=f"{self._selected_model() or 'importance'}_comparison.csv",
        )
        if not path:
            return
        ...
        try:
            ...
            return
        self._status_var.set(f"Exported {len(self._display_rows)} rows → {os.path.basename(path)}")
